...
from roshambo2.client import submit_search_multi_server, prepare_query
from rdkit import Chem

if __name__ == "__main__":

   # Define the URLs for the API endpoints in a list
    urls = ["http://localhost:8087/roshambo2/search1", 
            "http://localhost:8088/roshambo2/search2", 
            "http://localhost:8089/roshambo2/search3"]
    
    # load the query from SDF
    query_sdf = "../query.sdf"
    query_data = prepare_query(query_sdf, color=True)

    # submit the search to all servers 
    ...
    # number of hits as 100000 and tanimoto_threshold to 0.5.
    options={'optim_mode':'combination', 'max_results':10000, 'tanimoto_threshold': 0.5}
    
    
    print(options)

    results_dict, molecules = submit_search_multi_server(urls, query_data, options=options, get_structures=True)
    ...
    writer = Chem.SDWriter('best_fit_mols_from_multi_server.sdf')
    for mol in mol_list:
        writer.write(mol)
